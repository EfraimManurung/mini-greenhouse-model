...
class ServiceFunctions:
    ...
    def plot_all_data_nn(self, time, co2_in, temp_in, rh_in, PAR_in, par_in_predicted, temp_in_predicted, rh_in_predicted, co2_in_predicted):
        '''
        Plot all the parameters to make it easier to compare predicted vs actual values.

        Parameters:
        - time: List of time values
        - co2_in: List of actual CO2 values
        - temp_in: List of actual temperature values
        - rh_in: List of actual relative humidity values
        - PAR_in: List of actual PAR values
        - par_in_predicted: List of predicted PAR values
        - temp_in_predicted: List of predicted temperature values
        - rh_in_predicted: List of predicted relative humidity values
        - co2_in_predicted: List of predicted CO2 values
        '''
        ...
        data = [
            (co2_in, co2_in_predicted, 'CO2 In [ppm]'),
            (temp_in, temp_in_predicted, 'Temperature In [°C]'),
            (rh_in, rh_in_predicted, 'RH In [%]'),
            (PAR_in, par_in_predicted, 'PAR In [W/m2]')
        ]

        # Plot each dataset in a subplot
        for ax, (y_data, y_pred, title) in zip(axes.flatten(), data):
            ax.plot(time, y_data, label='Actual', color='blue')  # Plot actual data
            ax.plot(time, y_pred, label='Predicted', color='red', linestyle='--')  # Plot predicted data
    ...
    def export_to_excel_nn(self, filename, time, co2_in, temp_in, rh_in, PAR_in, par_in_predicted, temp_in_predicted, rh_in_predicted, co2_in_predicted):
        '''
        ...
        data = {
            ...
            'CO2 In (Actual)': co2_in,
            'CO2 In (Predicted)': co2_in_predicted,
            'Temperature In (Actual)': temp_in,
            'Temperature In (Predicted)': temp_in_predicted,
            'RH In (Actual)': rh_in,
            'RH In (Predicted)': rh_in_predicted,
            'PAR In (Actual)': PAR_in,
            'PAR In (Predicted)': par_in_predicted,
        }
